...
<div class="page-header-custom">
    <h1 class="page-title-custom">Daftar Pemilik</h1>
    <p class="page-subtitle-custom">Kelola data pemilik hewan</p>
</div>
            ...
            <table class="table-custom">
                ...
                <tbody>
                    @forelse($pemilik as $p)
                        <tr>
                            <td><span class="badge-id-custom">{{ $p->idpemilik }}</span></td>
                            <td>{{ $p->no_wa }}</td>
                            <td>{{ $p->alamat }}</td>
                            <td>{{ $p->user->name ?? '-' }}</td>
                            <td>
                                <div class="action-buttons-custom">
                                    <a href="{{ route('admin.pemilik.edit', $p->idpemilik) }}" class="btn-warning-custom">
    ...
    <div class="col-md-6 mb-3">
        <div class="stats-card-custom recent">
            <div>
                ...
                <h6 class="mb-0">{{ $pemilik->last()->idpemilik ?? '-' }}</h6>
            </div>
